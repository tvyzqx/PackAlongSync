...
//
// The rule under test: editable until the end of the trip's last day,
...
Deno.test("mid-trip stays editable — the window keys off the END, not the start", () => {
  // The decision that separates this rule from its predecessor: a trip running
  // Aug 1..Aug 10, checked on Aug 5, is open. Keying off start_date would have
  // shut it on Aug 1 and frozen the return-packing tab before anyone could use
  // it.
  assertEquals(
    withinEditWindow("2026-08-10T00:00:00+02:00", new Date("2026-08-05T12:00:00Z")),
    true,
  );
});

Deno.test("the last day itself is editable, including its last local hour", () => {
  // 23:30 Berlin on the last day = 21:30Z (CEST, +2).
  assertEquals(
...
Deno.test("end_date carrying a time of day does not cut the day short", () => {
  // Owner picked 09:00 local as the end; packing at 18:00 local still works.
  assertEquals(
...
Deno.test("day after the last day -> read-only", () => {
  assertEquals(
...
Deno.test("boundary is exact to the millisecond", () => {
  const end = "2026-08-01T00:00:00+02:00";
  // Aug 1 ends at 2026-08-01T21:59:59.999Z (00:00 Aug 2 Berlin, CEST +2).
  const lastMs = Date.UTC(2026, 7, 1, 21, 59, 59, 999);
  assertEquals(endOfDayUtcMs(new Date(end), BERLIN), lastMs);
  assertEquals(withinEditWindow(end, new Date(lastMs)), true);
  assertEquals(withinEditWindow(end, new Date(lastMs + 1)), false);
});
...
Deno.test("DST end: the 25-hour day ends at the right instant", () => {
  ...
  // 22:00Z, while the guest's clock still said 23:00 on the last day.
  const end = "2026-10-25T00:00:00+02:00";
  assertEquals(endOfDayUtcMs(new Date(end), BERLIN), Date.UTC(2026, 9, 25, 22, 59, 59, 999));
  assertEquals(withinEditWindow(end, new Date("2026-10-25T22:30:00Z")), true);
  assertEquals(withinEditWindow(end, new Date("2026-10-25T23:30:00Z")), false);
});
...
Deno.test("DST start: the 23-hour day ends at the right instant", () => {
  ...
  const end = "2026-03-29T00:00:00+01:00";
  assertEquals(endOfDayUtcMs(new Date(end), BERLIN), Date.UTC(2026, 2, 29, 21, 59, 59, 999));
  assertEquals(withinEditWindow(end, new Date("2026-03-29T21:30:00Z")), true);
  assertEquals(withinEditWindow(end, new Date("2026-03-29T22:30:00Z")), false);
});
